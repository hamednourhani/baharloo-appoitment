...
// 			if ($r != null) {
// //				$sms->to = array($r->phone);
// //				$sms->msg = $r->name." - ".$this->get_service_name( $r->service)." - ". $this->get_worker_name( $r->worker)." - ". $r->start;
// //           		 $sms->SendSMS();
// 				$message =  $r->name." - ".$this->get_service_name( $r->service)." - ". $this->get_worker_name( $r->worker)." - ". $r->start;
// 				var_dump($message);

// 				$headers = 'From: My Name <itstarir@gmail.com>' . "\r\n";
// 				wp_mail( 'itstarir@gmail.com', 'subject', $message, $headers );
// 				exit;
// 			}


	}
	function sms_cancel_appointment($app_id){
		global $wpdb,$sms;
...
//  	$msg = apply_filters('app_removal_notification_message', $msg, $app, $app_id);
		...
		if ($testSms) {
			$this->log(sprintf(__('Removal notification sms sent to %s for appointment ID:%s', 'my-plugin'), $email, $app_id));
		}
...
// 		$provider_add_text  = sprintf(__('An appointment removal sms for %s has been sent to your client:', 'my-plugin'), $app_id);
// 		$provider_add_text .= "\n\n\n";
...
*/
	...
	function get_monthly_calendar( $timestamp=false, $class='', $long, $widget ) {
		...
		for ($i=1; $i<=$days; $i++) {
			...
			$ret .= '<td class="'.$class_name.'" title="'.date_i18n($this->date_format, $ccs).'"><p>'.date_i18n($this->date_format, $ccs).'</p>
